...
static void LCD_Task(void *parameters)
{
	...
	LCD_Write(LCD_INSTRUCTION_REGISTER, (LCD_INSTRUCTION_FUNCTION_SET |
	LCD_FUNCTION_SET_DATA_INTERFACE_4_BITS) >> 4);
	vTaskDelay(pdMS_TO_TICKS(2));

	//Other LCD display microcontroller settings
	LCD_CommandInitiate(LCD_INSTRUCTION_REGISTER,
...
{
	LCD_CommandEnqueue(LCD_INSTRUCTION_REGISTER,
	LCD_INSTRUCTION_SET_DDRAM_ADDR | (addr & 0x7F));

	for (uint32_t i = 0; i < strlen(string); i++)
	{
		LCD_CommandEnqueue(LCD_DATA_REGISTER, string[i]);
	}
}
